...
sdm_model <- spml(
  log(precios) ~ DiD +
    ...
    time +
    log(loans) + log(wages) + log(popdensity) + log(housestock) + log(ageing_ratio) +
    log(ir) + log(crime_rate) + log(dwelling),
    data = df1,
  listw = listw,
  model = "within",
  lag = TRUE,
...
# Modelos con efectos fijos distintos
...
# Extraer resultados manualmente
...
modelsummary(list("1" = rmodelo1, "2" = rmodelo2, "3" = rmodelo3, "4" = rmodelo4),
             stars = FALSE,
             statistic = "p.value",
             output = "3. Results/tablas/modeloespacialg1.docx")
...
write.xlsx(resultados, "3. Results/tablas/modeloespacialg1.xlsx")

tabla <- flextable(resultados)
save_as_xlsx(tabla, path = "3. Results/tablas/modeloespacialg1.docx")
